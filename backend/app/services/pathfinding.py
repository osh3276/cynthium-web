...
class _Window:
    ...
    def round_offsets(self) -> "_Window":
        return _Window(
            round(self.col_off), round(self.row_off), self.width, self.height
        )

    def round_lengths(self) -> "_Window":
        return _Window(
            self.col_off, self.row_off, round(self.width), round(self.height)
        )


def _window_from_bounds(
    left: float, bottom: float, right: float, top: float, transform
) -> _Window:
...
def _snap_to_traversable(
    ...
    if (
        ...
        and 0 <= c < W
        and traversable[r, c]
        and _neighbor_count(r, c) >= _MIN_TRAV_NEIGHBORS
    ):
        return (r, c)
    ...
    while q:
        ...
        if (
            traversable[cr, cc]
            and d > 0
            and _neighbor_count(cr, cc) >= _MIN_TRAV_NEIGHBORS
        ):
            return (cr, cc)
        for dr, dc in dirs:
            nr, nc = cr + dr, cc + dc
...
def _compute_segment(
    *,
    ...
    if not (0 <= sc < W and 0 <= sr < H):
        return (
            None,
            f"start ({start_xy[0]:.1f}, {start_xy[1]:.1f}) outside site elevation",
        )
        ...
        if illum_window.width > 0 and illum_window.height > 0:
            ...
            if illum_crop.shape != elev_win.shape:
                illum_crop = zoom(
                    illum_crop,
                    (
                        elev_win.shape[0] / illum_crop.shape[0],
                        elev_win.shape[1] / illum_crop.shape[1],
                    ),
                    order=0,
                )
        else:
        ...
        if meteor_window.width > 0 and meteor_window.height > 0:
            ...
            if meteor_crop.shape != elev_win.shape:
                meteor_crop = zoom(
                    meteor_crop,
                    (
                        elev_win.shape[0] / meteor_crop.shape[0],
                        elev_win.shape[1] / meteor_crop.shape[1],
                    ),
                    order=0,
                )
        else:
