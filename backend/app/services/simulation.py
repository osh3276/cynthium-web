    ...
    for index in range(len(waypoints) - 1):
        ...
        for sample_index in range(segment_sample_count + 1):
            ...
            sampled_points.append(
                [float(current_xy[0]), float(current_xy[1]), elevation]
            )
...
def simulate_rover_over_path(
    *,
    ...
    Returns aggregated stats plus a ``path_velocity_profile`` array
    of ``[x, y, v_mps]`` triples for every sub-step point.
    """
...
def _add_context_stats(
    ...
    temperature_values = _sample_raster_values(
        points_xy, temperature_map, temperature_transform
    )
    ...
    illumination_values = _sample_raster_values(
        points_xy, illumination_map, illumination_transform
    )
...
def compute_traversal_score(stats: dict[str, float]) -> dict:
    ...
    path_eff = SCORE_MAX_PATH_EFFICIENCY * max(
        0.0, min(1.0, displacement / max(distance, 1.0))
    )
    ...
    illumination = SCORE_MAX_ILLUMINATION * max(
        0.0, min(1.0, 0.7 * illum_pct + 0.3 * solar_intensity)
    )
    ...
    if actual_mu <= 0.01:
        ...
        if ratio > 1.0:
            ...
            traction_match = SCORE_MAX_TRACTION_MATCH * max(
                0.0, min(1.0, ratio / TRACTION_PEAK_RATIO)
            )
    ...
    if v_flat_max <= 0.01:
        ...
        power_match = SCORE_MAX_POWER_MATCH * max(
            0.0, min(1.0, avg_v / max(v_ref * POWER_PEAK_RATIO, 0.01))
        )
    rover_opt = traction_match + power_match

    avg_meteor = stats.get("average_meteor_flux", 0.0)
    meteor_safety = SCORE_MAX_METEOR_SAFETY * max(
        0.0, min(1.0, 1.0 - avg_meteor / 5000.0)
    )
...
def _sanitize_stats(stats: dict) -> dict:
    ...
    for k, v in stats.items():
        if isinstance(v, str):
            ...
            sanitized[k] = {
                sk: _sanitize_float(sv) if isinstance(sv, (int, float)) else sv
...
def run_simulation(
    ...
    stats = calculate_path_stats(
        ...
        temperature_transform=rasters["temperature_meta"]["transform"]
        if rasters.get("temperature_meta")
        else None,
        illumination_map=rasters.get("illumination"),
        illumination_transform=rasters["illumination_meta"]["transform"]
        if rasters.get("illumination_meta")
        else None,
        meteor_map=rasters.get("meteor"),
        meteor_transform=rasters["meteor_meta"]["transform"]
        if rasters.get("meteor_meta")
        else None,
    )
